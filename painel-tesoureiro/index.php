...
//VARRER AS MENSALIDADES E VERIFICAR QUAIS DESSAS CONTAS JÁ ESTÃO COM O BOLETO PAGO PARA DAR BAIXA AUTOMATICAMENTE
$query = $pdo->query("SELECT * FROM pgto_matriculas where pago = 'Não' and (boleto != null or boleto != '') ");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
for($i=0; $i < $total_reg; $i++){
        foreach ($res[$i] as $key => $value){}
            ...
            if($status == 'paid'){
                $pdo->query("UPDATE pgto_matriculas set pago = 'Sim', data_pgto = curDate(), valor_pago = '$valor' where id = '$id'");

                //LANÇAR NAS MOVIMENTAÇÕES
                $pdo->query("INSERT movimentacoes SET tipo = 'Entrada', descricao = 'Pagamento Mensalidade', valor = '$valor', funcionario = '$cpf_usu', data = curDate(), mensalidade = 'Sim' ");
            }   
